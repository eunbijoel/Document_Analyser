...
def build_research_note_hwp(rows: Sequence[tuple[str, str]], *, title: str = "연구노트") -> bytes:
    ...
    # 템플릿 표 셀(값 열) 문단 인덱스 — Note_Template.hwp 기준
    label_to_para = {
        "주 제": 3,
        "주제": 3,
        "책 임 자": 5,
    ...
    for label, val in note_rows:
        key = label.strip()
        compact = key.replace(" ", "")
        idx = label_to_para.get(key) or label_to_para.get(compact)
        if idx is None:
            continue
        clean = _strip_md_noise(val)
        # 빈 문자열은 표 셀 조합 시 손상 유발 가능 → 공백 유지
        values[idx] = clean if clean.strip() else " "
